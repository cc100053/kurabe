...
- Core journeys:
  - Onboard & auth: Welcome screen → Supabase auth (guest, Google, Apple, email). Apple sign-in uses the iOS native flow (id token + nonce). Guests can start immediately; linking later migrates data. Email sign-up uses lazy registration (auto login) with a background verification email and a Profile settings nudge for unverified accounts. Unverified email users are blocked from Pro purchase until verification.
  - Capture/edit: FAB opens `AddEditScreen`; smart camera with guided overlay, Gemini 1.5 prompt extracts product/price/discount/category, user confirms quantity, tax rate, and price (two-way tax-exclusive/tax-inclusive inputs with ¥ + thousands formatting), plus price type/discount/category. The opposite field updates with a short debounce to avoid jitter while typing. Google Places nearby/Autocomplete powers shop selection; images upload to `price_tags`; Supabase save computes best-price flag via `get_nearby_cheapest`.
...
- Paywall expectations: RevenueCat entitlement `カイログ Pro`; package identifiers expected by the custom UI: `monthly`, `annual` (keep aligned with RC offerings). Non-Pro sees community counts; Pro sees full records. 初回14日無料トライアルを表示。
