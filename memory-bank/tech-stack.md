...
- Auth: Supabase email/password + OAuth (Google) + iOS native Sign in with Apple (`sign_in_with_apple` → id token + nonce → Supabase `signInWithIdToken`) + anonymous sessions; PKCE configured in `main.dart`; guest-to-user migration logic expected backend-side. Email verification uses Edge Functions to send a verification email and sets `user_metadata.email_verified` + `user_metadata.email_verified_at`.
- Subscriptions: RevenueCat (`purchases_flutter`, `purchases_ui_flutter`) with entitlement `カイログ Pro`; package IDs expected: `monthly`, `annual`; syncs to Supabase `profiles.is_pro`.
