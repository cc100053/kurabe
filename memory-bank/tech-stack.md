...
- Auth: Supabase email/password + OAuth (Google) + iOS native Sign in with Apple (`sign_in_with_apple` → id token + nonce → Supabase `signInWithIdToken`) + anonymous sessions; PKCE configured in `main.dart`; guest-to-user migration logic expected backend-side.
