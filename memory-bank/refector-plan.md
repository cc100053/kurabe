...
- （完成✅）Step 8 清理與死碼：標記或移除未用的 `YahooPlaceService` 等；評估逐步下線本地 SQLite，改用 Supabase 歷史。
- （完成✅）Step 9 測試與驗證：單元測試（稅/折扣/單價、去重/最安値、位置快取、訂閱錯誤映射）、Widget/Golden（Add/Edit 片段、共用 Tile）、整合測試（保存流程 stub）。
