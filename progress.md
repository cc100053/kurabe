...
- Auth & profile: Welcome gate with guest, Google, Apple, email. ProfileTab shows user info, hides logout for guests, offers link-with Google/Apple/email; email linking handles "email exists" by signing in and migrating guest data via `transfer_guest_data` RPC; guest reset button added. OAuth redirects unified to `io.supabase.flutter://login-callback/` via constant, PKCE enabled, iOS URL scheme registered; anonymous sessions detected via `user.isAnonymous` and signed out to show welcome.
